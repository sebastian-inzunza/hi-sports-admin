...
import MessageNotFound from '@/components/conversations/views/no-message-found'
...
const UserMessageView = ({
  ...
  if (loading) return <Loader className="!h-full flex-1" text="Cargando..." />
  // if (meError)
  //   return (
            ...
            {!isEmpty(messages) ? (
              <div className="space-y-6">
                {messages?.map((item: any, key: number) => {
                  ...
                  const checkUser = Number(1) === Number(user_id)
                  const avatarUrl = !permission
